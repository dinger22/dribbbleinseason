...
public class BucketListFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater,
        ...
        View view = inflater.inflate(R.layout.fragment_fab_swipe_recycler_view, container, false);
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        if (item.getItemId() == R.id.save) {
            ...
            for (Bucket bucket : (List<Bucket>)adapter.getData()) {
    ...
    private class NewBucketTask extends DribbleTasks<Void, Void, Bucket> {
        ...
        @Override
        protected Bucket doJob(Void... params) throws DribbleException {
            return DribbleUtils.newBucket(name, description);
        }
    ...
    private class LoadBucketsTask extends DribbleTasks<Void, Void, List<Bucket>> {
        ...
        @Override
        protected List<Bucket> doJob(Void... params) throws DribbleException {
            final int page = refresh ? 1 : adapter.getData().size() / DribbleUtils.SHOTS_PER_PAGE + 1;
            return userId == null
                    ? DribbleUtils.getUserBuckets(page)
                    : DribbleUtils.getUserBuckets(userId, page);
        }

        @Override
        protected void onSuccess(List<Bucket> buckets) {
            adapter.setShowLoading(buckets.size() >= DribbleUtils.SHOTS_PER_PAGE);
